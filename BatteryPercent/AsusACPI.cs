...
public class AsusACPI
{
    ...
    public AsusACPI()
    {
        ...
        {
            Debug.WriteLine($"Can't connect to ACPI: {ex.Message}");
        }
        ...
        if (AppConfig.IsAlly())
        {*/
        // Only supporting Ally right now for this functionality
            MaxTotal = 50;
            DefaultTotal = 30;
        /*}
    ...
    public int DeviceSet(uint DeviceID, int Status, string? logName)
    {
        ...
        if (logName is not null)
            Debug.WriteLine(logName + " = " + Status + " : " + (result == 1 ? "OK" : result));
    ...
    public int DeviceSet(uint DeviceID, byte[] Params, string? logName)
    {
        ...
        if (logName is not null)
            Debug.WriteLine(logName + " = " + BitConverter.ToString(Params) + " : " + (result == 1 ? "OK" : result));
    ...
    // We have no need to set any hardware related settings
    ...
    public (int, int) GetCores(bool max = false)
    {
        ...
        Debug.WriteLine("Cores" + (max ? "Max" : "") + ": 0x" + value.ToString("X4"));
    ...
    public void SetCores(int eCores, int pCores)
    {
        ...
        {
            Debug.WriteLine($"Incorrect Core config ({eCores}, {pCores})");
            return;
        };
    ...
    public void SubscribeToEvents(Action<object, EventArrivedEventArgs> EventHandler)
    {
        ...
        {
            Debug.WriteLine("Can't connect to ASUS WMI events");
        }
    }

    public String GetCurrentASUSMode()
    {
        // FIXME: Quick and dirty way to get this working
        Dictionary<string, object> config = new Dictionary<string, object>();
        config["performance_mode"] = 0;
        string jsonString = JsonSerializer.Serialize(config);

        if (config.ContainsKey("performance_mode"))
        {
            object mode = config.GetValueOrDefault("performance_mode");

            if (mode == null) return "Unknown Mode";
